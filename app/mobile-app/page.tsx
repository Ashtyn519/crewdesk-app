...
const TESTIMONIALS = [
  { name: 'Sarah Mitchell', role: 'Creative Director, StudioHive', text: 'The mobile app is incredible. I run my whole crew from my phone now. Absolute game changer.', rating: 5 },
  { name: 'James Okafor', role: 'Founder, PixelForge', text: 'Approving invoices on the go, checking crew status between takes. Cannot imagine working without it.', rating: 5 },
  ...
  return (
    <div className="min-h-screen bg-[#04080F] text-white overflow-x-hidden">
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#04080F]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Link href="/mobile-app" className="text-sm text-amber-400 font-semibold">Mobile App</Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/signup" className="text-sm font-semibold bg-amber-400 text-black px-4 py-2 rounded-lg hover:bg-amber-300 transition-colors">Start free</Link>
          </div>
        </div>
      </nav>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
          <div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-400/10 border border-amber-400/20 mb-8">
              ...
              <span className="text-xs font-semibold text-amber-400 uppercase tracking-wide">Now on iOS & Android</span>
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="text-5xl md:text-6xl font-black tracking-tight leading-[1.05] mb-6">
              Your workforce<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-amber-200">in your pocket</span>
            </motion.h1>
                ...
                <div className="flex items-center justify-between px-5 pt-8 pb-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <div className="w-5 h-2.5 rounded-sm border border-slate-400 relative">
                      <div className="absolute inset-0.5 left-0.5 right-1 bg-emerald-400 rounded-sm" />
                    </div>
                  </div>
                </div>
                <div className="px-4 pb-4 pt-1">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-white">CrewDesk</span>
                    </div>
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-[9px] font-bold text-black">A</div>
                  </div>
                  <p className="text-[10px] text-slate-500 mb-0.5">Dashboard</p>
                  <p className="text-sm font-bold text-white mb-4">Good morning</p>
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    {[{label:'Revenue',v:'48k',color:'text-amber-400'},{label:'Projects',v:'12',color:'text-blue-400'},{label:'Crew',v:'28',color:'text-purple-400'},{label:'Invoices',v:'5',color:'text-emerald-400'}].map(s => (
                      <div key={s.label} className="bg-[#0A1020] rounded-xl p-2.5 border border-[#1A2540]">
                        <div className={`text-base font-black mb-0.5 ${s.color}`}>{s.v}</div>
                        <div className="text-[9px] text-slate-500">{s.label}</div>
                      </div>
                    ))}
                  </div>
                  <p className="text-[9px] text-slate-500 mb-2 uppercase tracking-widest">Active Projects</p>
                  {[{name:'Brand Refresh',pct:72},{name:'App Prototype',pct:45},{name:'Marketing Push',pct:88}].map(p => (
                    <div key={p.name} className="flex items-center gap-2 mb-2">
                      <div className="text-[9px] text-slate-300 flex-1 truncate">{p.name}</div>
                  ...
                  <div className="absolute bottom-3 left-4 right-4 bg-[#060C18] border border-[#1A2540] rounded-2xl px-4 py-2 flex justify-around">
                    {[LayoutDashboard,FolderKanban,Receipt,Users].map((Icon,i) => (
                      <div key={i} className={`flex flex-col items-center gap-0.5 ${i===0?'text-amber-400':'text-slate-600'}`}>
      ...
      <section ref={featuresRef} className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-amber-400 font-semibold text-sm uppercase tracking-widest mb-3">Everything you need</motion.p>
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-4xl md:text-5xl font-black tracking-tight mb-4">One app. Total control.</motion.h2>
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-slate-400 text-lg max-w-2xl mx-auto">Every CrewDesk feature, optimised for mobile with a native feel and offline-first architecture.</motion.p>
          </div>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 mb-6">
              ...
              <span className="text-xs font-semibold text-blue-400 uppercase tracking-wide">Enterprise-grade security</span>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <motion.a href="https://apps.apple.com" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-2 px-5 py-3 bg-white text-black font-bold text-sm rounded-xl hover:bg-slate-100 transition-colors">
                <Apple className="w-4 h-4" />App Store
              </motion.a>
              <motion.a href="https://play.google.com" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.03 }} className="flex items-center gap-2 px-5 py-3 bg-[#0A1020] text-white font-semibold text-sm rounded-xl border border-[#1A2540] hover:border-[#243050] transition-colors">
            ...
            {[{icon:Shield,label:'Bank-grade encryption',color:'blue'},{icon:Wifi,label:'Offline-first sync',color:'green'},{icon:Bell,label:'Smart notifications',color:'amber'},{icon:Users,label:'Role-based access',color:'purple'}].map((item,i) => {
              ...
              return (
                <motion.div key={item.label} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="p-6 rounded-2xl bg-[#0A1020] border border-[#1A2540] text-center">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-3 ${bg} border ${border}`}>
                    <item.icon className={`w-5 h-5 ${c}`} />
                  </div>
                  <p className="text-xs font-semibold text-slate-300 text-center">{item.label}</p>
                </motion.div>
              );
            })}
          </motion.div>
      ...
      <section className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-black tracking-tight mb-3">Loved by creative teams</h2>
            <p className="text-slate-400">Join thousands of studios running their workforce on CrewDesk</p>
          </div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="p-12 rounded-3xl bg-gradient-to-br from-amber-400/10 to-amber-600/5 border border-amber-400/20 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl font-black tracking-tight mb-4">Ready to take CrewDesk mobile?</h2>
              <p className="text-slate-400 text-lg mb-8 max-w-2xl mx-auto">Download the app today. Free forever, upgrade when you are ready.</p>
              <div className="flex flex-wrap gap-4 justify-center mb-6">
                <motion.a href="https://apps.apple.com" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-3 bg-white text-black px-7 py-4 rounded-xl font-bold text-base hover:bg-slate-100 transition-colors shadow-lg">
                  <Apple className="w-5 h-5" />
                  <div className="text-left">
                    <div className="text-[10px] text-slate-500 leading-none">Download on the</div>
                ...
                <motion.a href="https://play.google.com" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-3 bg-[#0A1020] text-white px-7 py-4 rounded-xl font-bold text-base border border-[#1A2540] hover:border-[#243050] transition-colors shadow-lg">
                  <Play className="w-5 h-5" />
                  <div className="text-left">
                    <div className="text-[10px] text-slate-500 leading-none">Get it on</div>
              ...
              <Link href="/signup">
                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors">
                  Or start on web for free <ArrowRight className="w-4 h-4" />
                </motion.button>
              </Link>
            </div>
          </motion.div>
      ...
      <footer className="border-t border-[#1A2540] py-10 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-600 text-sm ml-2">© 2026. All rights reserved.</span>
          </div>
