...
function SettingsRow({ label, desc, children }: { label: string; desc?: string; children: React.ReactNode }) {
    return (
          <div className="flex items-center justify-between py-3 border-b border-white/5 last:border-0">
                <div>
                        <p className="text-sm text-white">{label}</p>p>
                        ...
                        <main className="flex-1 overflow-y-auto p-6">
                                  <div className="max-w-2xl mx-auto">
                                              <div className="mb-8">
                                                            ...
                                                            <p className="text-slate-500 text-sm mt-1">Manage your account and workspace preferences</p>p>
                                              </div>div>
                          ...
                          <div>
                                          <SettingsSection title="Personal Information">
                                                            ...
                                                            <div className="mb-4">
                                                                                <label className="block text-xs text-slate-400 mb-1">Email Address</label>label>
                                                                                ...
                                                                                <p className="text-xs text-slate-600 mt-1">Email cannot be changed here. Contact support to update.</p>p>
                                                            </div>div>
                                            {saveError && <p className="text-xs text-rose-400 mb-3">{saveError}</p>p>}
                          ...
                          <div>
                                          <SettingsSection title="Workspace Details">
                                                            <div className="mb-4">
                                                                                <label className="block text-xs text-slate-400 mb-1">Workspace Name</label>label>
                          ...
                          <div>
                                          <SettingsSection title="Current Plan">
                                                            <div className="flex items-center justify-between p-4 rounded-xl border border-amber-500/30 mb-4 bg-[#04080F]">
                                                                                <div>
                                                                                                      <p className="text-sm font-semibold text-white">{workspace ? getPlanLabel(workspace.plan) : '—'}</p>p>
                                                                                  {workspace?.plan === 'trial' ? (
                                                    <p className="text-xs text-amber-400 mt-0.5">{getTrialDaysLeft() !== null ? `${getTrialDaysLeft()} days remaining` : 'Trial active'}</p>p>
                                                  ) : (
                                                    <p className="text-xs text-slate-400 mt-0.5 capitalize">{workspace?.subscription_status || 'Active'}</p>p>
                                                                                                      )}
                                                                                  </div>div>
                                            ...
                                            {workspace?.plan === 'trial' ? (
                                                <div className="space-y-3">
                                                                      <p className="text-sm text-slate-400">Upgrade to a paid plan to keep full access after your trial ends.</p>p>
                                                ...
                                                <div className="space-y-3">
                                                                      <p className="text-sm text-slate-400">Manage your subscription, update payment methods, and download invoices through the billing portal.</p>p>
                                                  ...
                                                  {billingError && (
                                                                          <div className="flex items-start gap-2 p-3 rounded-lg bg-rose-500/10 border border-rose-500/20">
                                                                                                    ...
                                                                                                    <p className="text-xs text-rose-400">{billingError}</p>p>
                                                                          </div>div>
                                                                      )}
                                                </div>div>
                                                            )}
                                          ...
                                          <SettingsSection title="Need help?">
                                                            <p className="text-xs text-slate-500">Questions about billing?{' '}
                                                                                <a href="mailto:info@crewdeskapp.com" className="text-amber-400 hover:text-amber-300 transition-colors">Contact us at info@crewdeskapp.com</a>a>
                                                            </p>p>
                                          </SettingsSection>SettingsSection>
                          </div>div>
                                              )}
                          ...
                          <div>
                                          <SettingsSection title="Password">
                                                            <p className="text-sm text-slate-400 mb-3">To reset your password, use the link below. We&apos;ll send a secure reset email to your address.</p>p>
                                          ...
                                          <SettingsSection title="Sign Out Everywhere">
                                                            <p className="text-sm text-slate-400 mb-3">Sign out from all devices and sessions.</p>p>
