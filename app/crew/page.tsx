...
export default function FreelancersPage() {
  ...
  async function addCrewMember() {
    ...
    if (!error && data) setCrew(prev => [...prev, data])
    setSaving(false)
    setShowAdd(false)
    ...
    <div className="flex min-h-screen bg-[#04080F]">
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          {!loading && crew.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="text-white font-semibold mb-1">No freelancers yet</h3>
              <p className="text-white/30 text-sm mb-6 max-w-xs">Add your first freelancer to start building your crew roster.</p>
              <button
                onClick={() => { setFormData(EMPTY_FORM); setShowAdd(true) }}
                className="flex items-center gap-2 bg-amber-400 hover:bg-amber-300 text-black px-4 py-2 rounded-xl text-sm font-semibold transition-colors"
              >
                <Plus className="w-4 h-4" />
                Add your first freelancer
              </button>
            </div>
          )}
              ...
              {filtered.map(f => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/5">
                    ...
                    <span className="text-amber-400 text-xs font-semibold">{f.rate ? `£${f.rate}/hr` : '—'}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          >
            ...
            <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl mb-5">
              <div className="text-center">
                <p className="text-amber-400 font-bold">{selected.rate ? `£${selected.rate}/hr` : '—'}</p>
      ...
      {(showAdd || editMode) && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-white font-bold text-lg">{editMode ? 'Edit Freelancer' : 'Add Freelancer'}</h2>
              <button
                onClick={() => { setShowAdd(false); setEditMode(false); setEditTarget(null) }}
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                >
                  <option value="" className="bg-[#04080F]">Department</option>
                  {DEPARTMENTS.filter(d => d !== 'All').map(d => (
                    <option key={d} value={d} className="bg-[#04080F]">{d}</option>
                  ))}
                </select>
                <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40 pointer-events-none" />
              </div>
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-white/5 border border-white/10 text-white rounded-xl px-4 py-2.5 pr-8 text-sm focus:outline-none"
                >
                  <option value="available" className="bg-[#04080F]">Available</option>
                  <option value="on-project" className="bg-[#04080F]">On Project</option>
                  <option value="unavailable" className="bg-[#04080F]">Unavailable</option>
                </select>
                <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40 pointer-events-none" />
              </div>
