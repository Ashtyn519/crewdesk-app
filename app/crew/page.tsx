...
interface CrewMember {
  ...
  rating: number; status: 'Available' | 'On Project' | 'Unavailable'; projects: number;
  skills: string[]; initials: string; color: string; joined: string
}
...
function StarRating({ rating, onChange }: { rating: number; onChange?: (r: number) => void }) {
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(s => (
        <span key={s} onClick={() => onChange?.(s)} className={`text-sm ${onChange ? 'cursor-pointer' : ''} ${s <= rating ? 'text-amber-400' : 'text-gray-600'}`}>★</span>
      ))}
    </div>
  )
}
    ...
    <motion.div layout whileHover={{ y: -2 }} className="bg-[#0A1020] border border-white/[0.06] rounded-xl p-5 cursor-pointer hover:border-amber-400/20 transition-all group" onClick={onClick}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${member.color} flex items-center justify-center text-white font-bold text-sm`}>{member.initials}</div>
          <div>
            <h3 className="text-white font-semibold text-sm">{member.name}</h3>
            <p className="text-gray-500 text-xs">{member.role}</p>
          </div>
        </div>
        <span className={`text-xs px-2 py-0.5 rounded-full ${STATUS_STYLE[member.status]}`}>{member.status}</span>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {member.skills.slice(0, 3).map(s => <span key={s} className="text-xs px-2 py-0.5 bg-white/5 text-gray-400 rounded-full">{s}</span>)}
        {member.skills.length > 3 && <span className="text-xs px-2 py-0.5 bg-white/5 text-gray-500 rounded-full">+{member.skills.length - 3}</span>}
      </div>
      <div className="flex justify-between items-center pt-3 border-t border-white/5 text-xs">
        <span className="text-gray-500">💰 £{member.rate}/{member.rateUnit}</span>
    ...
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
      <motion.div className="bg-[#0A1020] border border-white/10 rounded-2xl p-8 w-full max-w-lg shadow-2xl" initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-4 mb-6">
          <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${m.color} flex items-center justify-center text-white font-bold text-xl`}>{m.initials}</div>
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs text-gray-500">{m.rating}/5</span>
            </div>
          </div>
          <span className={`text-xs px-3 py-1 rounded-full ${STATUS_STYLE[m.status]}`}>{m.status}</span>
        </div>
        <div className="grid grid-cols-2 gap-4 mb-5">
          <div><label className="text-xs text-gray-400 mb-1 block">Email</label><input className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-amber-400/50 focus:outline-none" value={m.email} onChange={e => setM({...m, email: e.target.value})} /></div>
          <div><label className="text-xs text-gray-400 mb-1 block">Day Rate (£)</label><input type="number" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-amber-400/50 focus:outline-none" value={m.rate} onChange={e => setM({...m, rate: Number(e.target.value)})} /></div>
          <div><label className="text-xs text-gray-400 mb-1 block">Status</label>
            <select className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-amber-400/50 focus:outline-none" value={m.status} onChange={e => setM({...m, status: e.target.value as CrewMember['status']})}>
              {['Available', 'On Project', 'Unavailable'].map(s => <option key={s} value={s}>{s}</option>)}
            </select></div>
          <div><label className="text-xs text-gray-400 mb-1 block">Projects Done</label><p className="text-white font-semibold mt-2">{m.projects}</p></div>
        </div>
        <div className="mb-5"><label className="text-xs text-gray-400 mb-2 block">Skills</label>
          <div className="flex flex-wrap gap-1.5">{m.skills.map(s => <span key={s} className="text-xs px-3 py-1 bg-white/5 text-gray-300 rounded-full border border-white/10">{s}</span>)}</div>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={() => { onUpdate(m); onClose() }} className="flex-1 py-2.5 rounded-lg bg-amber-400 text-black font-semibold text-sm hover:bg-amber-300 transition-colors">Save Changes</button>
        </div>
      </motion.div>
    </motion.div>
  )
      ...
      <motion.div className="bg-[#0A1020] border border-white/10 rounded-2xl p-8 w-full max-w-md shadow-2xl" initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={e => e.stopPropagation()}>
        {sent ? (
          <div className="text-center py-4">
            ...
            <p className="text-gray-400 text-sm mb-6">An invitation has been sent to {email}</p>
            <button onClick={onClose} className="w-full py-2.5 rounded-lg bg-amber-400 text-black font-semibold text-sm">Done</button>
          </div>
        ) : (
          <>
            <h2 className="text-xl font-bold text-white mb-2">Invite Crew Member</h2>
            <p className="text-gray-400 text-sm mb-6">They&apos;ll receive an email to join your CrewDesk workspace.</p>
            <div className="space-y-4 mb-6">
              ...
              <div><label className="text-xs text-gray-400 mb-1 block">Role</label><input className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:border-amber-400/50 focus:outline-none" value={role} onChange={e => setRole(e.target.value)} placeholder="e.g. Camera Operator" /></div>
            </div>
            <div className="flex gap-3">
              <button onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-white/10 text-gray-400 text-sm hover:bg-white/5 transition-colors">Cancel</button>
              <button onClick={() => setSent(true)} className="flex-1 py-2.5 rounded-lg bg-amber-400 text-black font-semibold text-sm hover:bg-amber-300 transition-colors">Send Invite</button>
            </div>
          </>
        )}
      </motion.div>
...
export default function CrewPage() {
  ...
  const filtered = crew.filter(m => (filter === 'All' || m.status === filter) && (m.name.toLowerCase().includes(search.toLowerCase()) || m.role.toLowerCase().includes(search.toLowerCase())))
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <button onClick={() => setInvite(true)} className="flex items-center gap-2 bg-amber-400 hover:bg-amber-300 text-black font-semibold text-sm px-4 py-2 rounded-lg transition-colors">
              <span className="text-base leading-none">+</span> Invite Crew
            </button>
          </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20 text-gray-500">
              ...
              <p className="font-medium">No crew found</p>
              <p className="text-sm mt-1">Try a different filter or invite new crew</p>
            </div>
          )}
        </main>
      </div>
