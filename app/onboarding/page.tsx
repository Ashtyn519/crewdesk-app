...
export default function OnboardingPage() {
  ...
  // When we reach 'done', redirect after a short delay so the user sees it
  useEffect(() => {
        if (step === 'done') {
  ...
  async function finish() {
        ...
        try {
          ...
          // Show the done step first, then redirect (handled by useEffect above)
          setStep('done')
        } catch (e: any) {
          ...
          {/* Progress */}
              <div className="w-full max-w-lg mb-8">
                      <div className="flex items-center justify-between mb-2">
                                <span className="text-xs text-slate-500">Step {stepIndex + 1} of {STEPS.length}</span>span>
                                <span className="text-xs text-slate-500">{Math.round(progress)}% complete</span>span>
                      </div>div>
              ...
              <div className="w-full max-w-lg">
                ...
                {step === 'workspace' && (
                    <div className="bg-[#0A1020] border border-white/5 rounded-2xl p-8">
                                ...
                                <h1 className="text-2xl font-bold text-white mb-2">Name your workspace</h1>h1>
                                <p className="text-slate-400 text-sm mb-8">This is usually your company or agency name. You can change it later.</p>p>
                                <div>
                                              <label className="block text-xs font-medium text-slate-400 mb-2">Workspace name</label>label>
                                              <input
                                                                autoFocus
                                                                type="text"
                                                                value={workspaceName}
                ...
                {step === 'industry' && (
                    <div className="bg-[#0A1020] border border-white/5 rounded-2xl p-8">
                                ...
                                <h1 className="text-2xl font-bold text-white mb-2">What best describes your business?</h1>h1>
                                <p className="text-slate-400 text-sm mb-6">We'll customise your experience to suit your industry.</p>p>
                                <div className="grid grid-cols-2 gap-2.5">
                                  {INDUSTRIES.map(ind => (
                                      <button
                                                        ...
                                                        >
                                                        <span className="text-lg">{ind.icon}</span>span>
                                                        <span className="text-xs font-medium leading-snug">{ind.label}</span>span>
                                ...
                                <div className="flex gap-3 mt-6">
                                              <button
                                                                ...
                                                                className="flex items-center gap-2 px-4 py-3 rounded-xl border border-white/10 text-slate-400 hover:text-white text-sm transition-all"
                                                              >
                ...
                {step === 'team' && (
                    <div className="bg-[#0A1020] border border-white/5 rounded-2xl p-8">
                                ...
                                <h1 className="text-2xl font-bold text-white mb-2">Tell us about your team</h1>h1>
                                <p className="text-slate-400 text-sm mb-6">Helps us set sensible defaults for your workspace.</p>p>
                                <div className="mb-6">
                                              <p className="text-xs font-medium text-slate-400 mb-3">How many people in your team?</p>p>
                                        ...
                                        <button
                                                            ...
                                                            >
                                                            <p className={`text-sm font-semibold ${teamSize === ts.id ? 'text-amber-400' : 'text-white'}`}>{ts.label}</p>p>
                                                            <p className="text-[11px] text-slate-500 mt-0.5">{ts.desc}</p>p>
                                        </button>button>
                                      ))}
                                              </div>div>
                                </div>div>
                                <div className="mb-6">
                                              <p className="text-xs font-medium text-slate-400 mb-3">What do you mainly want to use CrewDesk for?</p>p>
                                        ...
                                        <button
                                                            ...
                                                            <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${
                                                                                    useCases.includes(uc.id) ? 'bg-amber-400 border-amber-400' : 'border-white/20'
                                                            }`}>
                                                              {useCases.includes(uc.id) && <Check className="w-3 h-3 text-black" />}
                                                            </div>div>
                                                            <span className="text-sm">{uc.label}</span>span>
                                        </button>button>
                                      ))}
                                              </div>div>
                                </div>div>
                      {error && (
                                    <p className="text-rose-400 text-sm mb-4 bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-3">{error}</p>p>
                                )}
                ...
                {step === 'done' && (
                    <div className="bg-[#0A1020] border border-white/5 rounded-2xl p-12 text-center">
                                ...
                                <h1 className="text-2xl font-bold text-white mb-2">You're all set!</h1>h1>
                                <p className="text-slate-400 text-sm">Taking you to your dashboard…</p>p>
