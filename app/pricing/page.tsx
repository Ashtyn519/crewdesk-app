        ...
        <div className="min-h-screen bg-[#04080F]">
              <nav className="flex items-center justify-between px-8 py-5 border-b border-white/5">
                      <Link href="/" className="flex items-center gap-2">
                                <div className="w-8 h-8 bg-amber-400 rounded-lg flex items-center justify-center"><Zap className="w-4 h-4 text-black" fill="black" /></div>div>
                                <span className="text-white font-bold text-lg">CrewDesk</span>span>
                      </Link>Link>
                      <div className="flex items-center gap-4">
                                <Link href="/login" className="text-slate-400 hover:text-white text-sm transition">Sign in</Link>Link>
                                <Link href="/signup" className="bg-amber-400 hover:bg-amber-300 text-black text-sm font-semibold px-4 py-2 rounded-xl transition">Get started</Link>Link>
                      </div>div>
              </nav>nav>
              <div className="max-w-5xl mx-auto px-6 py-20">
                      <div className="text-center mb-16">
                                <h1 className="text-4xl font-bold text-white mb-4">Simple, transparent pricing</h1>h1>
                                <p className="text-slate-400 text-lg max-w-xl mx-auto">Start with a 14-day free trial. No credit card required. Cancel anytime.</p>p>
                      </div>div>
                {error && <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 text-sm text-center py-3 px-4 rounded-xl mb-8 max-w-md mx-auto">{error}</div>div>}
                      ...
                      <div key={plan.id} className={`relative rounded-2xl p-8 flex flex-col ${plan.highlight ? 'bg-amber-400/5 border-2 border-amber-400/40' : 'bg-[#0A1020] border border-white/5'}`}>
                        {'badge' in plan && plan.badge && (
                                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-400 text-black text-xs font-bold px-3 py-1 rounded-full">{plan.badge}</div>div>
                                    )}
                                    <div className="mb-6">
                                                    <h3 className="text-lg font-semibold text-white">{plan.name}</h3>h3>
                                                    <p className="text-slate-400 text-sm mt-1">{plan.description}</p>p>
                                                    <div className="mt-4 flex items-baseline gap-1">
                                                                      <span className="text-4xl font-bold text-white">£{plan.price}</span>span>
                                                                      <span className="text-slate-400 text-sm">/month</span>span>
                                                    </div>div>
                                    </div>div>
                      ...
                      <p className="text-center text-slate-500 text-sm mt-12">
                                ...
                                <Link href="/login" className="text-amber-400 hover:text-amber-300">Already have an account?</Link>Link>
                      </p>p>
              </div>div>
        </div>div>
      )
