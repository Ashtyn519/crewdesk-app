...
export default function MessagesPage() {
  ...
  const loadThreads = useCallback(async () => {
    ...
    if (!user) return
    const { data: ws } = await sb.from('workspaces').select('id').eq('owner_id', user.id).single()
  ...
  const sendMessage = async () => {
    ...
    const { data: newMsg } = await sb.from('chat_messages').insert({
      thread_id: activeId, text, sender: 'me', sent_at: now, read: true,
    }).select().single()
    if (newMsg) setMessages(prev => [...prev, newMsg])
    await sb.from('message_threads').update({ last_message: text, last_message_at: now }).eq('id', activeId)
    setThreads(prev => prev.map(t => t.id === activeId ? { ...t, last_message: text, last_message_at: now } : t))
    setSending(false)
  }
  ...
  const createThread = async () => {
    ...
    if (data) { setThreads(prev => [data, ...prev]); setActiveId(data.id) }
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold text-white truncate">{thread.contact_name}</p>
                      <div className="flex items-center gap-1">
                        <span className="text-xs text-slate-500 shrink-0">{formatTime(thread.last_message_at)}</span>
                        <button
          ...
          {activeThread ? (
            <div className="flex-1 flex flex-col min-w-0">
              <div className="flex items-center gap-3 px-5 py-3 border-b border-white/5 bg-[#0A1020]">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">{activeThread.contact_name}</p>
                  <p className="text-xs text-slate-400">{activeThread.contact_role}</p>
                </div>
              </div>
              ...
              <div className="px-5 py-3 border-t border-white/5 bg-[#0A1020]">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="w-9 h-9 flex items-center justify-center bg-amber-400 hover:bg-amber-300 disabled:opacity-50 text-black rounded-xl transition-colors"
                  >
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowCompose(false)} className="text-white/40 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-amber-400 hover:bg-amber-300 disabled:opacity-50 text-black font-semibold py-2.5 rounded-xl text-sm transition-colors"
              >
                Start Conversation
              </button>
            </div>
      ...
      {deleteConfirm && (
        ...
        >
          <div
            ...
            onClick={e => e.stopPropagation()}
          >
            <h2 className="text-white font-bold mb-2">Delete conversation?</h2>
            <p className="text-white/40 text-sm mb-5">This will permanently delete the conversation and all its messages.</p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteConfirm(null)}
                className="flex-1 bg-white/5 border border-white/10 text-white/60 py-2.5 rounded-xl text-sm hover:bg-white/10 transition-colors"
              ...
              <button
                ...
                className="flex-1 bg-rose-500 hover:bg-rose-400 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors"
              >
                Delete
              </button>
            </div>
