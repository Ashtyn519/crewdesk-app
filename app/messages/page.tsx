...
export default function MessagesPage() {
  ...
  const loadMessages = useCallback(async (threadId: string) => {
        ...
        setLoadingMessages(false)
                                       ...
                                       // Mark as read
                                       await sb.from('messages').update({ read: true }).eq('thread_id', threadId).eq('sender', 'them')
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                          ...
                          <div
                                            ...
                                            <div className="w-8 h-8 rounded-full bg-amber-400/10 border border-amber-400/20 flex items-center justify-center shrink-0 mt-0.5">
                                                                <span className="text-amber-400 font-bold text-xs">
                                                                  {thread.contact_name.split(' ').map(n => n[0]).join('')}
                                                                </span>span>
                                            </div>div>
                      ...
                      <div className="flex-1 flex flex-col min-w-0">
                                    <div className="flex items-center gap-3 px-5 py-3 border-b border-white/5 bg-[#0A1020]">
                                                    <div className="w-8 h-8 rounded-full bg-amber-400/10 border border-amber-400/20 flex items-center justify-center">
                                                                      <span className="text-amber-400 font-bold text-xs">
                                                                        {activeThread.contact_name.split(' ').map(n => n[0]).join('')}
                                                                      </span>span>
                                                    </div>div>
                                                    <div>
                                                                  ...
                                                                  msg.sender === 'me'
                                                                    ...
                                                                    : 'bg-[#0F1A2E] text-white rounded-bl-sm border border-white/5'
                                                                                    ...
                                                                                    <div className={`flex items-center gap-1 mt-1 ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                                                                                      ...
                                                                                      {msg.sender === 'me' && (
                                                                      msg.read
                                                                        ? <CheckCheck size={11} className="text-black/60" />
                                                                        : <Check size={11} className="text-black/60" />
                                                                    )}
                                                                                      </div>div>
                                                              </div>div>
                                          </div>div>
                                    ...
                                    <div className="px-5 py-3 border-t border-white/5 bg-[#0A1020]">
                                                    <div className="flex items-center gap-3">
                                                                      <input
                                                                                            ...
                                                                                            placeholder={`Message ${activeThread.contact_name}...`}
                                                                                            className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-400/50"
                                                                                          />
                                                                      <button
                                                                                            onClick={sendMessage}
                                                                                            disabled={!input.trim() || sending}
                                                                                            className="p-2.5 bg-amber-400 text-black rounded-xl hover:bg-amber-300 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                                                                                          >
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
                                    ...
                                    <h3 className="text-white font-semibold mb-1">
                                      {loading ? 'Loading…' : threads.length === 0 ? 'No conversations yet' : 'Select a conversation'}
                                    </h3>h3>
                                    <p className="text-white/30 text-sm mb-6">
                                      {!loading && threads.length === 0 ? 'Start a new conversation with a freelancer.' : 'Choose a conversation from the sidebar.'}
                                    </p>p>
                        {!loading && threads.length === 0 && (
                                        <button
                                                            onClick={() => setShowCompose(true)}
                  ...
                  <div
                            ...
                            <div
                                        ...
                                        <div className="flex items-center justify-between mb-5">
                                                      <h2 className="text-base font-bold text-white">New Conversation</h2>h2>
                                        ...
                                        <div className="space-y-3 mb-5">
                                                      <div>
                                                                      ...
                                                                      <input
                                                                                          ...
                                                                                          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-400/50"
                                                                                        />
                                                      </div>div>
                                                      <div>
                                        ...
                                        <button
                                                        ...
                                                        className="w-full py-2.5 bg-amber-400 text-black font-bold rounded-xl text-sm hover:bg-amber-300 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                                                      >
                                                      Start Conversation
                                        </button>button>
                            </div>div>
          ...
          {deleteConfirm && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                            <div className="bg-[#0A1020] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                                        <h3 className="text-base font-bold text-white mb-2">Delete Conversation</h3>h3>
                                        <p className="text-sm text-slate-400 mb-6">This will permanently remove this thread and all messages. Are you sure?</p>p>
                                        <div className="flex gap-3">
                                                      <button
                                                                        onClick={() => setDeleteConfirm(null)}
                                                                        className="flex-1 py-2.5 border border-white/10 text-slate-300 rounded-xl text-sm hover:bg-white/5 transition-colors"
                                                                      >
                                                                      Cancel
                                                      </button>button>
                                                      <button
                                                                        onClick={() => deleteThread(deleteConfirm)}
                                                                        className="flex-1 py-2.5 bg-rose-500 text-white font-semibold rounded-xl text-sm hover:bg-rose-600 transition-colors"
                                                                      >
                                                                      Delete
                                                      </button>button>
                                        </div>div>
