      ...
      <section className="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-400/10 border border-amber-400/20 mb-8">
            ...
            <span className="text-xs font-semibold text-amber-400 uppercase tracking-wide">Now available on iOS & Android</span>
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="text-5xl md:text-7xl font-black tracking-tight leading-[1.05] mb-6">
            The operating system <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-amber-200">for your freelance workforce</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-xl text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/signup">
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-2 px-8 py-4 bg-amber-400 text-black font-bold text-base rounded-xl hover:bg-amber-300 transition-colors shadow-lg shadow-amber-400/20">
                Start for free <ArrowRight className="w-4 h-4" />
              </motion.button>
            </Link>
            <Link href="/login">
              <motion.button whileHover={{ scale: 1.02 }} className="flex items-center gap-2 px-8 py-4 bg-white/5 text-white font-semibold text-base rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
                View demo
              </motion.button>
            </Link>
          </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-8 flex items-center justify-center gap-2 text-sm text-slate-500">
      ...
      <section ref={featuresRef} className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-4xl md:text-5xl font-black tracking-tight mb-4">One platform. Total control.</motion.h2>
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-slate-400 text-lg max-w-2xl mx-auto">Stop juggling spreadsheets, emails, and invoicing tools. CrewDesk brings it all under one roof.</motion.p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 mb-6">
                ...
                <span className="text-xs font-semibold text-blue-400 uppercase tracking-wide">iOS & Android</span>
              </div>
              <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-6">Your workforce in <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-amber-200">your pocket</span></h2>
              <p className="text-slate-400 text-lg leading-relaxed mb-8">The full power of CrewDesk, optimised for mobile. Review projects, approve invoices, message your crew, and track contracts anywhere, anytime.</p>
              <div className="space-y-3 mb-10">
                {['Real-time notifications','Full invoice management','Crew availability','Offline sync'].map(item => (
              ...
              <div className="flex flex-wrap gap-3">
                <Link href="/mobile-app">
                  <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="flex items-center gap-2 px-5 py-3 bg-white text-black font-bold text-sm rounded-xl hover:bg-slate-100 transition-colors">
                    <Apple className="w-4 h-4" />App Store
                  </motion.button>
                </Link>
                <Link href="/mobile-app">
                  <motion.button whileHover={{ scale: 1.03 }} className="flex items-center gap-2 px-5 py-3 bg-[#0A1020] text-white font-semibold text-sm rounded-xl border border-[#1A2540] hover:border-[#243050] transition-colors">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
              <div className="relative mx-auto w-64 h-[500px] rounded-[40px] bg-[#0A1020] border-2 border-[#1A2540] p-3 shadow-2xl shadow-blue-500/10">
                ...
                <div className="w-full h-full rounded-[32px] bg-[#04080F] overflow-hidden p-4 pt-8">
                  <div className="text-[10px] text-slate-500 mb-3">Dashboard</div>
                  <div className="text-sm font-bold text-white mb-4">Good morning</div>
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    {[{label:'Projects',v:'12',c:'amber'},{label:'Crew',v:'28',c:'blue'},{label:'Invoices',v:'5',c:'yellow'},{label:'Earned',v:'48k',c:'green'}].map(s => (
                      <div key={s.label} className="bg-[#0A1020] rounded-xl p-2.5 border border-[#1A2540]">
                        <div className={`text-base font-black mb-0.5 ${s.c==='amber'?'text-amber-400':s.c==='blue'?'text-blue-400':s.c==='green'?'text-emerald-400':'text-yellow-400'}`}>{s.v}</div>
                        <div className="text-[9px] text-slate-500">{s.label}</div>
                      </div>
                    ))}
                  </div>
                  <div className="text-[10px] text-slate-500 mb-2">Active Projects</div>
                  {['Brand Refresh','App Prototype','Marketing'].map((p,i) => (
                    <div key={p} className="flex items-center gap-2 mb-2">
                      <div className="text-[9px] text-slate-300 flex-1 truncate">{p}</div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="p-12 rounded-3xl bg-gradient-to-br from-amber-400/10 to-amber-600/5 border border-amber-400/20 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl font-black tracking-tight mb-4">Ready to take control?</h2>
              <p className="text-slate-400 text-lg mb-8 max-w-2xl mx-auto">Start managing your freelance workforce like a world-class operation. Free forever, upgrade when you are ready.</p>
              <Link href="/signup">
                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="inline-flex items-center gap-2 px-8 py-4 bg-amber-400 text-black font-bold text-base rounded-xl hover:bg-amber-300 transition-colors shadow-lg shadow-amber-400/20">
