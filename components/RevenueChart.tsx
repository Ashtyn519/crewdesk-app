    ...
    return(
          <div>
                <div className="flex items-start justify-between mb-5">
                        <div>
                                  <div className="flex items-center gap-3 mb-0.5">
                                              <span className="text-2xl font-black text-amber-400">£{(total/1000).toFixed(1)}k</span>
                                              <span className="text-emerald-400 text-xs font-bold px-2 py-0.5 rounded-full bg-emerald-400/10 border border-emerald-400/20">+23.4%</span>
                                  </div>
                                  <p className="text-xs text-slate-500">vs last year</p>
                        </div>
                        <div className="flex gap-1">
                          {(['1M','3M','YTD','1Y']as Period[]).map(p=>(
                        <button key={p} onClick={()=>setPeriod(p)} className={period===p?'text-xs px-2.5 py-1.5 rounded-lg font-medium bg-amber-400/10 text-amber-400 border border-amber-400/20':'text-xs px-2.5 py-1.5 rounded-lg font-medium text-slate-500 hover:text-slate-300'}>{p}</button>
                      ))}
                        </div>
                </div>
                ...
                <div className="flex gap-1.5">
                  {months.map(m=><div key={m} className="flex-1 text-center text-[10px] text-slate-600 font-medium">{m}</div>)}
                </div>
          </div>
        )
}
