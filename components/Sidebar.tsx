          ...
          <div className="flex flex-col h-screen w-64 bg-[#060C18] border-r border-[#1A2540] fixed left-0 top-0 z-40">
                ...
                <div className="mx-4 mt-3 mb-2 px-3 py-3 rounded-xl bg-[#0A1020] border border-[#1A2540] hover:border-amber-400/20 transition-colors cursor-pointer">
                        <div className="flex items-center gap-2">
                                  <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-xs font-bold text-black shrink-0">W</div>div>
                                  <div className="flex-1 min-w-0">
                                              <p className="text-xs font-semibold text-white truncate">My Workspace</p>p>
                                              <p className="text-[10px] text-slate-500">Trial &middot; 11 days left</p>p>
                                  </div>div>
                                                            ...
                                                            <Link key={item.href} href={item.href}>
                                                                          <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer relative ${isActive ? 'bg-amber-400/10 text-amber-400' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                                                                            ...
                                                                            {item.badge !== null && item.badge > 0 && (
                                                                                <span className="text-[10px] font-bold bg-amber-400 text-black rounded-full px-1.5 py-0.5 min-w-[18px] text-center leading-none">{item.badge}</span>span>
                                                                                          )}
                                                                          </div>div>
                                                            </Link>Link>
                                                          )
