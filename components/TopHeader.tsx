  ...
  return (
        <header className="h-14 border-b border-white/5 flex items-center justify-between px-6 sticky top-0 z-40" style={{ background: 'rgba(4,8,15,0.95)', backdropFilter: 'blur(12px)' }}>
                <div className="flex items-center gap-2 text-sm">
                        <span className="text-slate-500">CrewDesk</span>span>
                        <span className="text-slate-600">›</span>span>
                        <span className="text-white font-medium">{pageName}</span>span>
                </div>div>
        
              <div className="flex items-center gap-3">
                {/* Search */}
                      <div className="relative">
                                <motion.div animate={{ width: searchOpen ? 240 : 36 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }} className="flex items-center bg-white/5 border border-white/10 rounded-xl overflow-hidden h-9">
                        ...
                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }} className="absolute top-12 right-0 w-72 rounded-2xl border border-white/10 py-2 z-50 shadow-2xl" style={{ background: '#0A1020' }}>
                                        <p className="text-xs text-slate-500 px-4 py-1 mb-1 uppercase tracking-wider">Results</p>p>
                          {filtered.map((r, i) => {
                                                                ...
                                                                return (
                                                                                      <Link key={i} href={r.href}>
                                                                                                            <div className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/5 cursor-pointer transition-colors">
                                                                                                                                    ...
                                                                                                                                    <div>
                                                                                                                                                              <p className="text-sm font-semibold text-white">{r.label}</p>p>
                                                                                                                                                              <p className="text-xs text-slate-500">{r.sub}</p>p>
                                                                                                                                      </div>div>
                                                                                                              </div>div>
                                                                                        </Link>Link>
                                                                                    )
                ...
                {/* Notifications */}
                      <div className="relative" ref={notifRef}>
                                <button onClick={() => setNotifOpen(!notifOpen)} className="relative w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/8 border border-white/10 transition-all text-slate-400 hover:text-white">
                                  ...
                                  {unreadCount > 0 && (
                        <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-1 -right-1 w-4 h-4 bg-amber-500 rounded-full text-[10px] font-bold text-black flex items-center justify-center">
                          {unreadCount}
                                ...
                                <AnimatePresence>
                                  {notifOpen && (
                        <motion.div initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 4, scale: 0.96 }} className="absolute top-12 right-0 w-80 rounded-2xl border border-white/10 z-50 shadow-2xl overflow-hidden" style={{ background: '#0A1020' }}>
                                        <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
                                                          <p className="text-sm font-semibold text-white">Notifications</p>p>
                                                          <button onClick={markAllRead} className="text-xs text-amber-400 hover:text-amber-300">Mark all read</button>button>
                                        </div>div>
                                                                                          ...
                                                                                          return (
                                                                                                                  <div key={n.id} className={`flex items-start gap-3 px-4 py-3 hover:bg-white/5 transition-colors cursor-pointer border-b border-white/5 last:border-0 ${n.unread ? 'bg-amber-500/[0.03]' : ''}`}>
                                                                                                                                          ...
                                                                                                                                          <div className="flex-1">
                                                                                                                                                                    <p className={`text-sm leading-snug ${n.unread ? 'text-white font-medium' : 'text-slate-400'}`}>{n.text}</p>p>
                                                                                                                                                                    <p className="text-xs text-slate-500 mt-1">{n.time}</p>p>
                                                                                                                                            </div>div>
                                        ...
                                        <div className="px-4 py-2 border-t border-white/5">
                                                          <button className="w-full text-center text-xs text-amber-400 hover:text-amber-300 py-1">View all notifications</button>button>
                                        </div>div>
                        </motion.div>motion.div>
                      )}
                                </AnimatePresence>AnimatePresence>
