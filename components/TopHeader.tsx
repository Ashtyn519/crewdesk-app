  ...
  return (
        <header className="h-14 border-b border-white/5 flex items-center justify-between px-6 sticky top-0 z-40" style={{ background: 'rgba(4,8,15,0.95)', backdropFilter: 'blur(12px)' }}>
                <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-slate-600">&rsaquo;</span>span>
                        <span className="text-white font-medium">{pageName}</span>span>
                </div>div>
        
              <div className="flex items-center gap-3">
                {/* Notifications — placeholder, shows empty state */}
                      <button
                      ...
                      <div className="absolute right-0 top-10 w-52 bg-[#0F1A2E] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
                        {(userName || userEmail) && (
                                        <div className="px-4 py-3 border-b border-white/5">
                                          {userName && <p className="text-xs font-semibold text-white truncate">{userName}</p>p>}
                                          {userEmail && <p className="text-xs text-slate-500 truncate">{userEmail}</p>p>}
                                        </div>div>
                                    )}
